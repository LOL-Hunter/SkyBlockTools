...
import tksimple as tk, iterDict, ID
...
class BazaarCraftProfitPage(CustomPage):
    def __init__(self, master):
        ...
        self.recursiveCraft.setText("Add-Deep-Recipes").setSelected()
        ...
        self.validBzItems = [i._getID() for i in self.validRecipes]
    ...
    def _getValidRecipes(self):
        ...
        for recipe in RecipeAPI.getRecipes():
            if not self.isBazaarItem(recipe._getID()): continue # filter Items to only take Bazaar Items
            ...
            for ingredient in ingredients:
                ...
                if not self.isBazaarItem(indName): # filter ingredients
                    if recipe._getID() not in self.forceAdd:
    ...
    def getRecipeResults(self, itemID:str, resultPrice, factor:int)->List[RecipeResult]:
        ...
        def _getRec(_itemID):
            ...
            recipe = RecipeAPI.getRecipeFromID(_itemID)
        ...
        variants.append(
            RecipeResult(itemID, profitPerCraft * factor, craftPrice * factor, requiredItemString, depth)
        )
        """
    ...
    def updateTreeView(self):
        ...
        recipeList = []
        for recipe in self.validRecipes[0:1]:
            result = recipe._getID()

            result = BazaarItemID.WHALE_BAIT.value # TEMP !!!!!!!!!
            recipe = RecipeAPI.getRecipeFromID(BazaarItemID.WHALE_BAIT)

            if self.searchE.getValue() != "":
                if recipe._getID() not in validItems: continue
            ...
            results = self.getRecipeResults(result, resultPrice, factor)

            recipeList.extend(results)
        recipeList.sort()
...
class BazaarFlipProfitPage(CustomPage):
    def __init__(self, master):
        ...
        self.headerIndex = "Profit-Per-Item"
        self.headerToKey = {
            "Item":"ID",
        ...
        self.showStackProfit = tk.Checkbutton(self.settingsWindow, SG)
        self.showStackProfit.setText("Show-Profit-as-Stack[x64]").setTextOrientation()
        self.showStackProfit.placeRelative(fixHeight=25, fixWidth=200, fixY=50)
        ...
        self.includeEnchantments.placeRelative(fixHeight=25, fixWidth=200, fixY=75)
        ...
        self.showOthersTry.placeRelative(fixHeight=25, fixWidth=200, fixY=100)
        ...
        self.perHour.placeRelative(fixHeight=25, fixWidth=200, fixY=125)
        ...
        self.hideLowInstaSell.placeRelative(fixHeight=25, fixWidth=200, fixY=150)

        self.showProfitPerHour = tk.Checkbutton(self.settingsWindow, SG)
        self.showProfitPerHour.setText("Show-Flip-Profit_per-Hour").setTextOrientation()
        self.showProfitPerHour.placeRelative(fixHeight=25, fixWidth=200, fixY=175)
    ...
    def updateTreeView(self):
        ...
        if not self.showStackProfit.getValue():
            factor = 1
            titles = ["Item", "Profit-Per-Item", "Buy-Price", "Sell-Price"]
        else:
            factor = 64
...
class BazaarToAuctionHouseFlipProfitPage(CustomPage):
    ...
    def _getValidRecipes(self):
        ...
        for recipe in RecipeAPI.getRecipes():
            ...
            if not self.isAuctionItem(recipe._getID()): continue # filter Items to only take Auction Items
            validIngredient = True
            ingredients = recipe.getItemInputList()
            if ingredients is None: # no recipe available
                continue
                ...
                for auct_ID, active in zip(*API.SKYBLOCK_AUCTION_API_PARSER.getBinTypeAndAuctions()):
                    ...
                    for auction in active:
                        ...
                        if auction.getCreatorUUID() in ownAuctionUUIDs.keys():  # own Auction
                            ...
                            ownAuctionIDs.add(auction._getID())
                        sorters.append(
                            Sorter(
                                sortKey="price".lower(),

                                name=auction._getID() + ("" if pName is None else f" ({pName})"),
...
class Window(tk.Tk):
    ...
    def showItemInfo(self, page:CustomPage, itemID:str):
        page.openNextMenuPage(self.additionalItemInfoPage, itemName=itemID)
